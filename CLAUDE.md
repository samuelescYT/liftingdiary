...
**ALL code generated by Claude Code MUST ALWAYS first refer to the relevant documentation file(s) within the `/docs` directory.** Before writing or modifying any code, read the applicable doc(s) in `/docs` and follow their conventions. Do not generate code before doing so.
